...
CREATE TABLE sleep_session (
    ...
    minutes_asleep  INT4        GENERATED ALWAYS AS (EXTRACT(EPOCH FROM (stop_time - start_time)) / 60) STORED,
    notes           VARCHAR     NOT NULL DEFAULT '',
    CONSTRAINT stop_after_start CHECK (stop_time >= start_time)
);

--     minutes_asleep  INT4        GENERATED ALWAYS AS ((EXTRACT(EPOCH FROM (stop_time - start_time)) / 60) - out_of_bed) STORED,
